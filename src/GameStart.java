...
public class GameStart implements ApplicationListener {
	...
	public void render() {
		...
		// This is important because neither of them would move with the camera
		// without this.
		...
		// Update the Tracking Camera with the players rectangle as it moves
		// around.
		Camera.UpdateCameraTracking();

		// Updates the Players interaction with the level.
		Levelgen.UpdateDetection(shap, Player1);

		// Renders the entire level
		// ** Also Renders the PLAYER
		// This is needed for both the Level and the Player to be rendered
		Levelgen.RenderLevel(shap);

		// This is last because it needs to put an overlay over everything and
		// not let anything appear over top of it.
